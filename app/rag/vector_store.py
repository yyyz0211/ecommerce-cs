...
def get_chroma_client(persist_dir: Optional[str] = None):
    ...
    Chroma is imported lazily so the app can still start before the optional
    dependency is installed or before the vector index is built.
    """
    import chromadb
...
def query_documents(
    ...
    *,
    ...
    for doc_id, text, metadata, distance in zip(ids, docs, metadatas, distances):
        # Chroma cosine distance is lower-is-better. This derived score is only
        # for display/filtering, not a calibrated probability.
