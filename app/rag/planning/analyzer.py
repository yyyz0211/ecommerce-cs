...
def analyze_query(query: str, *, category: Optional[str] = None) -> QueryAnalysis:
    """为第一版 RAG 构建确定性的 query 分析结果。

    这里是后续接入 LLM query 改写的边界：以后可以让模型填充 `rewrite_query`
    或增强 `keywords`，其余检索链路仍保持同一份数据契约。
    """
